 ...
 *
 *	\Note		
 * objectifs : bouton 1 (marche / arret)
 * fonctionnel : oui
 * tests : -
 * limite : -
 * \ cablage : Port D sur les lettres du 7 segment et C0, C1, C6 et C7 pour Dis 0,1,2,3 et B0 avec BP0
 */
...
void  __interrupt(high_priority) myHighint(void)             // High priority interrupt
{
   ...
   * { */
    ...
    {
        if (stop==1) //si est a 1 on arrete
        {
            stop=0;
        }
        else //sinon on met en marche
        {
            stop=1;
        }
        INTCONbits.INT0IF = 0;
