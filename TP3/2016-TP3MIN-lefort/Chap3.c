 ...
 * 
 * Test : Affichage de BO sur l'écran puis affichage de BO et NJ en passant 40 cases pour un retour à la ligne
 * 
 * ///////////////////////Fonctionnel ///////////////////////////////
 */
...
char etat,flag,test,var3, var1, var2, var4, flag=0;
...
void main(void) {
    ...
    init();//initialisation de l'écran et du pic
    for(i=0;i<4;i++){
        code_tape[i] = digit();
    }
    
    for(i=0;i<4;i++){
        if (code[i]!= code_tape[i]){
            flag1=1;
...
void ecrire(char donne, char param){
    if (param != 2){
        busy();//verifie que l'ecran a terminé sa tache precedente
    }
    ...
    if (param == 1){
        RS=1;//mode donnée (pour ecrire sur l'ecran)
        RW=0;
    }
    TRISD=0;
...
void busy(){//verifie si l'ecran a terùiné la demande precedente et atte,d que ce soit le cas
    char valeur =0x80;
    while(valeur==0x80){
